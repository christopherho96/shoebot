...
try:
    ...
    time.sleep(1)
    browser.find_element_by_xpath("//*[@class='option-tiles__items']/*[@title='8.5']/span[text()='8.5']").click()
    time.sleep(1)
    ...
    browser.find_element_by_xpath("//input[@name='firstName']").send_keys(info[0])
    browser.find_element_by_xpath("//input[@name='lastName']").send_keys(info[1])
    browser.find_element_by_xpath("//input[@name='line1']").send_keys(info[2])
    browser.find_element_by_xpath("//input[@name='town']").send_keys(info[3])
    browser.find_element_by_xpath("//select[@id='province']/option[text()='{}']".format(info[4])).click()
    browser.find_element_by_xpath("//input[@name='postalCode']").click()
    browser.find_element_by_xpath("//input[@name='postalCode']").send_keys(info[5])
    browser.find_element_by_xpath("//input[@name='email']").send_keys(info[6])
    browser.find_element_by_xpath("//input[@name='phone']").click()
    browser.find_element_by_xpath("//input[@name='phone']").send_keys(info[7])
    ...
    time.sleep(2)
    browser.find_element_by_xpath("//input[@id='card_number']").send_keys(info[8])
    browser.find_element_by_xpath("//input[@id='expiry_date_text']").send_keys(info[9])
    browser.find_element_by_xpath("//input[@id='cvd_input']").send_keys(info[10])
